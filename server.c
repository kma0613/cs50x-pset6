...
char* indexes(const char* path)
{
    ...
    while (n--)
    {
        if (strcmp(namelist[n] -> d_name, "index.html") == 0)
        {
            char* p = strdup(path);
            strcat(p, "index.html");
            return p;
        }
        else if (strcmp(namelist[n] -> d_name, "index.php") == 0)
        {
            char* p = strdup(path);
            strcat(p, "index.php");
...
{
    int memAlloc = 10;
    char* heap = malloc(memAlloc * sizeof(char));
    int size = 0;
...
const char* lookup(const char* path)
{
    ...
    else if (strcasecmp(p, ".jpg") == 0)
    {
        mimeType = "image/jpeg";
    }
    else if (strcasecmp(p, ".js") == 0)
    {
        mimeType = "text/javascript";
    }
    else if (strcasecmp(p, ".php") == 0)
    {
        mimeType = "text/x-php";
    }
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    else if (strstr(line, "HTTP/1.1") == NULL)
    {
        ...
        {
            int qLength = strlen(q) - strlen(temp) - 1;
            strncpy(query, q + 1, qLength);
            query[qLength] = '\0';
        }

        if (strcmp(query, "") == 0)
        {
            int rtLength = strlen(a) - strlen(temp);
            strncpy(abs_path, a, rtLength + 1);
            abs_path[rtLength] = '\0';
        }
        else
        {
            int rtLength = strlen(a) - strlen(q);
            strncpy(abs_path, a, rtLength + 1);
            abs_path[rtLength] = '\0';
        }
